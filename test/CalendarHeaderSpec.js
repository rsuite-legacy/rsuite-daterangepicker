...
describe('Calendar-Header', () => {
  ...
  it('Should call `onMoveForword` callback', done => {
    ...
    const format = 'YYYY-MM';
    const instance = ReactTestUtils.renderIntoDocument(
      <Header date={date} format={format} onMoveForword={doneOp} />
    );
    const instanceDOM = findDOMNode(instance);
  ...
  it('Should call `onMoveBackward` callback', done => {
    ...
    const format = 'YYYY-MM';
    const instance = ReactTestUtils.renderIntoDocument(
      <Header date={date} format={format} onMoveBackward={doneOp} />
    );
    const instanceDOM = findDOMNode(instance);
  ...
  it('Should call `onToggleMonthDropdown` callback', done => {
    ...
    const format = 'YYYY-MM';
    const instance = ReactTestUtils.renderIntoDocument(
      <Header date={date} format={format} onToggleMonthDropdown={doneOp} />
    );
    const instanceDOM = findDOMNode(instance);
